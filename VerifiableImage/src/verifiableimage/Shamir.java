...
public class Shamir {
    ...
    public static BigInteger[][] divide(int[] SecV,int []SecS,int k,int n,BigInteger p)
    {
        ...
        for(int m=0;m<SecS.length;m++)
        {
            for(int i=0;i<n;i++)
            {
                BigInteger temp= BigInteger.valueOf(SecS[m]);
                BigInteger x;
                for(int j=0;j<k-1;j++)
                {
    ...
    public static int[] join(int[] SecV,BigInteger[][] y,int k, BigInteger p)
    {
